        ...
        <v-flex xs12>
          ...
          <v-data-table
            ...
            <template
              ...
              <td>
                {{ new Date(item.appDate).toDateString() }}
                -{{ new Date (item.appDate).toLocaleTimeString() }}
              </td>
                    ...
                    <div
                      ...
                      <v-flex slot="header">
                        <v-tabs
                          ...
                          >Tasks:</span>
                          <v-tab class="mr-3">
                            <v-icon class="mr-2">mdi-bug</v-icon>Sameer
                          </v-tab>
                          <!-- <v-tab class="mr-3">
                ...
                <v-tab-item>
                  ...
                  >
                    ...
                    <template
                      ...
                      <td class="text-xs-center">{{ props.item.pPrice*props.item.selectUnit.qty*props.item.pQty }}</td>
...
export default {
  data () {
    return {
      requestHeader: [
        ...
        { sortable: false, text: 'Order Approved Date', value: 'date' },
      ...
      orderdItem: [],
      orderheaders: [
        { sortable: false, text: '....', value: 'action', align: 'left', width: '2%' },
  ...
  methods: {
    getRequests () {
      orderAPi.getOrders('Ordered').then(res => {
        if (res) {
          this.orders = res
        }
      })
    ...
    order () {
      ...
      this.selected.forEach(el => {
        var index = this.items.map((o) => o.itemId).indexOf(el._id)
    ...
    callit () {
      ...
      this.totalQty = 0
      this.selected.forEach(el => {
        this.totalPrice += el.pQty * el.pPrice * el.selectUnit.qty
        this.totalQty += el.pQty
      })
    },
    /** End */
    getPermissions () {
