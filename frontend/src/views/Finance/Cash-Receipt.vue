<template>
  <v-container
    ...
    <v-layout justify-center>
      <v-flex md8>
        <v-card>
          <v-card-title class="headline">Create Cash Receipt Voucher</v-card-title>
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              @click="cashReceive(obj);">Save</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
    </v-layout>
...
export default {
  ...
  methods: {
    ...
    cashReceive () {
      api.cashReceive(this.obj).then(resp => {
        this.receipt = true
        if (resp === true) {
          this.item = { ...this.obj }
