  ...
  <v-container>
    ...
    <v-dialog
      ...
      <v-form v-model="valid">
        <v-card>
          <v-card-title>
            <span class="headline">Amakin Form</span>
          </v-card-title>
          <v-card-text>
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            color="warning"
            flat
            @click="deleteAmakin()">Delete</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  data () {
    return {
      ...
      search: null,
      amakins: [],
      form: constants.amakin_FORM,
      item_headers: constants.amakin_HEADERS,
      newObj: {},
      editObj: {},
      delObj: {},
      isNew: false,
  ...
  methods: {
    ...
    getAmakins () {
      othersApi.getAmakinApi().then(res => {
        this.amakins = res
      })
    },
    addAmakin () {
      othersApi.addAmakinApi(this.newObj).then(res => {
    ...
    editAmakin () {
      othersApi.editAmakinApi(this.editObj).then(res => {
        if (res.status === 'success') {
          this.dialog = false
    ...
    deleteAmakin () {
      othersApi.deleteAmakinApi(this.delObj['_id']['$oid']).then(res => {
        if (res.status === 'success') {
          this.delObj = {}
